...
public class Rscode implements Fec {
    ...
    public Rscode() {
        ...
        rs = new byte[allNum][stripe_unit_size];
        inthis = new BitSet();
    }
    ...
    public void encoding() {
        ...
        buffer = new byte[n][stripe_unit_size];
        for (int i = 0; i < n; i++) {
        ...
        for (int iByte = 0; iByte < stripe_unit_size; iByte++) {
            for (int iRow = num; iRow < allNum; iRow++) {
                int value = 0;
                for (int c = 0; c < num; c++) {
                    value ^= multiply((byte) vdm[iRow * num + c], rs[c][iByte]);
                }
                rs[iRow][iByte] = (byte) value;
            }
        }
    }
    public void decoding() {
        ...
        buff = new byte[allNum][stripe_unit_size];
        ...
        map = new int[rows];
        buffer = new byte[allNum][stripe_unit_size];
        ...
        for (int iByte = 0; iByte < stripe_unit_size; iByte++) {
            for (int iRow = 0; iRow < cols; iRow++) {
                if (id[iRow] >= cols) {
                    ...
                    int value = 0;
                    for (int c = 0; c < num; c++) {
                        value ^= multiply((byte) inv[iRow * num + c],
                                buffer[map[c]][iByte]);
                    }
                    rs[iRow][iByte] = (byte) value;
                }
    ...
    public void decoding(int[] map, byte[][] buffer) {
        int[] vdm;
        Condensed_Matrix cm;
        int rows, cols;
        int m, n;
        ...
        for (int iByte = 0; iByte < stripe_unit_size; iByte++) {
            for (int iRow = 0; iRow < cols; iRow++) {
                if (id[iRow] >= cols) {
                    int value = 0;
                    for (int c = 0; c < num; c++) {
                        value ^= multiply((byte) inv[iRow * num + c], buffer[map[c]][iByte]);
                    }
                    rs[iRow][iByte] = (byte) value;
                }
            }
    ...
    Condensed_Matrix gf_condense_dispersal_matrix(int[] disp, int[] existing_rows, int rows, int cols) {
        Condensed_Matrix cm;
        int[] m;
        int[] id;
        int i, j, k, tmp;
    ...
    public static void main(String[] args) {
        ...
        int allNum = 9;
        int rsNum = 4;
        int num = allNum - rsNum;
        int stripe_unit_size = 10;
        // all data is 10, checksum data is 3, data stripe length is 1024
        Rscode rsItem = new Rscode(allNum, rsNum, stripe_unit_size);
        byte[][]  enrs= rsItem.getRs();
        for (int i = 0; i < num; i++) {
            for (int j = 0; j < 10; j++) {
                if (i == 2){
                    if (j > 5){
        ...
        for (int l = 0; l < 10; l++) {
            buffer[map[3]][l] = rs[3][l];
        }

        for (int l = 0; l < 10; l++) {
            buffer[map[4]][l] = rs[4][l];
        }

        for (int l = 0; l < 10; l++) {
            buffer[map[5]][l] = rs[5][l];
        }
        ...
        for (int l = 0; l < 10; l++) {
            rstemp[3][l]=buffer[map[3]][l] ;
        }
        for (int l = 0; l < 10; l++) {
            rstemp[4][l]=buffer[map[4]][l] ;
        }
        rsItem.setRs(rstemp);
        rsItem.outputOrigin();
    }
